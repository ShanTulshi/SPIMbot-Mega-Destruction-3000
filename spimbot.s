...
move_up_to_water:
	li	$s1, 1
	sw	$s1, ANGLE_CONTROL				#make sure angle is absolute rather than relative	
	li	$s1, 270			
	sw	$s1, ANGLE 					#set angle to 270
	li	$s1, 10
	sw	$s1, VELOCITY 					#make velocity 1
	lw	$s1, BOT_Y
	bge	$s1, 75, move_up_to_water
	j	open_valve
